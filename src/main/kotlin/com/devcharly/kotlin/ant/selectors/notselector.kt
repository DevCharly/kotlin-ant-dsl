...
interface INotSelectorNested : INestedComponent {
	fun not(
		error: String? = null,
		nested: (KNotSelector.() -> Unit)? = null)
	{
		_addNotSelector(NotSelector().apply {
			component.project.setProjectReference(this);
			_init(error, nested)
		})
	}

	fun _addNotSelector(value: NotSelector)
}

fun NotSelector._init(
	error: String?,
	nested: (KNotSelector.() -> Unit)?)
{
	if (error != null)
		setError(error)
	if (nested != null)
		nested(KNotSelector(this))
}

class KNotSelector(override val component: NotSelector) :
	IFileSelectorNested,
