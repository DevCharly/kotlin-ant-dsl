...
interface IMajoritySelectorNested : INestedComponent {
	fun majority(
		allowtie: Boolean? = null,
		error: String? = null,
		nested: (KMajoritySelector.() -> Unit)? = null)
	{
		_addMajoritySelector(MajoritySelector().apply {
			component.project.setProjectReference(this);
			_init(allowtie, error, nested)
		})
	}

	fun _addMajoritySelector(value: MajoritySelector)
}

fun MajoritySelector._init(
	allowtie: Boolean?,
	error: String?,
	nested: (KMajoritySelector.() -> Unit)?)
{
	if (allowtie != null)
		setAllowtie(allowtie)
	if (error != null)
		setError(error)
	if (nested != null)
		nested(KMajoritySelector(this))
}

class KMajoritySelector(override val component: MajoritySelector) :
	IFileSelectorNested,
