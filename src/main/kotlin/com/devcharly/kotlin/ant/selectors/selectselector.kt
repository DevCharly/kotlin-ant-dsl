...
interface ISelectSelectorNested : INestedComponent {
	fun selector(
		If: String? = null,
		unless: String? = null,
		error: String? = null,
		nested: (KSelectSelector.() -> Unit)? = null)
	{
		_addSelectSelector(SelectSelector().apply {
			component.project.setProjectReference(this);
			_init(If, unless, error, nested)
		})
	}

	fun _addSelectSelector(value: SelectSelector)
}

fun SelectSelector._init(
	If: String?,
	unless: String?,
	error: String?,
	nested: (KSelectSelector.() -> Unit)?)
{
	if (If != null)
		setIf(If)
	if (unless != null)
	...
	if (error != null)
		setError(error)
	if (nested != null)
		nested(KSelectSelector(this))
}

class KSelectSelector(override val component: SelectSelector) :
	IFileSelectorNested,
